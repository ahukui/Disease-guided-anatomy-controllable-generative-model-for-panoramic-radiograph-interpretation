...
from PR_dataset import PRGenDataset
...
def validate(vae, test_dataloader, device, kld_weight: float, logdir: str, step: int):
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        loss_dict = vae_compute_loss(kld_weight, vae(image, sample_posterior=True))
        ...
        if batch_idx < 8:  # Adjust based on how many batches you want to save
            recon_images = vae(image, sample_posterior=True)[0]
    ...
    if images_to_save:
        ...
        save_image(torch.abs(original_images - reconstructed_images), os.path.join(logdir, f'different_{step}.png'), nrow=4)
...
def train(rank, world_size, config):
    ...
    train_MRI_dataset = PRGenDataset(data_path="XX/ExtractedDicom/", mode='train', stage='vae')
    val_MRI_dataset = PRGenDataset(data_path="XX/ExtractedDicom/", mode='val', stage='vae')
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=config.train_batch_size, sampler=train_sampler, pin_memory=True, num_workers=4)
    val_dataloader = DataLoader(val_dataset, batch_size=config.val_batch_size, pin_memory=True, num_workers=4, shuffle=False)

    if config.continue_train_path:
        vae = AutoencoderKL.from_pretrained(config.continue_train_path, subfolder='')
    else:
        vae = AutoencoderKL.from_config(config.pretrained_model_path, subfolder='')
    ...
    for step in range(config.train_steps):
        ...
        if rank == 0:
            ...
            if (step + 1) % config.validation_steps == 0:
                torch.cuda.empty_cache()
                val_loss = validate(vae, val_dataloader, device, config.kld_weight, logdir, step)
                torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    config = OmegaConf.load("./training_configs/train_vae_config_1024_512.yml")
