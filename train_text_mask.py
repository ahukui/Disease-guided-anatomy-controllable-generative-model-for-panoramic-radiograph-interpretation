...
class SampleLogger:
    ...
    def log_sample_images(self, batch, pipeline: StableDiffusionControlNetPipeline, device: torch.device, step: int):
        ...
        for idx, prompt in enumerate(tqdm(self.prompts, desc="Generating sample images")):
            ...
            skeleton = batch["skeleton"][idx, :, :, :].unsqueeze(0).to(device=device)

            merged_mask = batch["mask"][idx, :, :, :].unsqueeze(0)
            ...
            IM = (image+ 1.) / 2. # for visualization
            IM = IM.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            mask = (mask+ 1.) / 2. # for visualization
            mask = mask.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            skeleton = (skeleton + 1.) / 2. # for visualization
            skeleton = skeleton.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            recon_img = (recon_out + 1.) / 2. #[:, 0:1, :, :]
            recon_img = recon_img.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
            recon_img = np.clip(recon_img, 0, 1)

            IM = np.clip(IM, 0, 1)

            cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}.png"), IM * 255)
            cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}_mask.png"), mask* 255)
            cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}_skeleton.png"), skeleton* 255)
            cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}_output.png"), recon_img* 255)

            with open(os.path.join(self.logdir, f"{step}_{idx}" + '.txt'), 'a') as f:
                f.write(batch['prompt'][idx])
            # for i, img in enumerate(sequence):
            #     #print('~~~~~~~~~~~~~', img.shape)
            #     img = (img + 1.) / 2. # for visualization
            #     img =  img.detach().cpu().numpy()
            #     img = np.clip(img, 0, 1)    

            #     cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}_{sample_seeds[i]}_output.png"), img[0, :,:]* 255)
            #     cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}_{sample_seeds[i]}_Mask_output.png"), img[1, :,:]* 255)
...
def train(
    ...
    mixed_precision: str = "fp16", #Optional[str] = "fp16",
    train_batch_size: int = 1,
    val_batch_size: int = 1,
    learning_rate: float = 3e-5,
    scale_lr: bool = False,
    lr_scheduler: str = "constant_with_warmup",  # ["linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup"]
    ...
    train_dataset = PRGenDataset(data_path=data_json, mode='train')
    val_dataset = PRGenDataset(data_path=data_json, mode='val')

    print(train_dataset.__len__())
    print(val_dataset.__len__())
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=train_batch_size, num_workers=16, pin_memory=True, shuffle=True)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=val_batch_size, num_workers=16, pin_memory=True, shuffle=False)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("PRGen")
    step = 0
    ...
    while step < train_steps:
        ...
        skeleton = batch["skeleton"].to(accelerator.device, dtype=weight_dtype)
        prompt = batch["prompt"]
        ...
        down_block_res_samples, mid_block_res_sample = controlnet(noisy_latent, timesteps, encoder_hidden_states=encoder_hidden_states, controlnet_cond=skeleton, return_dict=False)
        ...
        loss = F.mse_loss(model_pred.float(), noise.float(), reduction="mean") + F.l1_loss(model_pred.float(), noise.float(), reduction="mean")
...
if __name__ == "__main__":
    config = "./training_configs/train_controlnet_config.yml"  #"/mnt/rds/VipinRDS/VipinRDS/users/cxs965/QK/MRGen/two_mask_train_unet/PRGen_251031-221848/config.yml"
