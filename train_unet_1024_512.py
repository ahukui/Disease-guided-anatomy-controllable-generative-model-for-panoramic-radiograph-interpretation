...
class SampleLogger:
    ...
    def log_sample_images(self, batch, pipeline: StableDiffusionPipeline, device: torch.device, step: int):
        ...
        for idx, prompt in enumerate(tqdm(self.prompts, desc="Generating sample images")):
            ...
            # IM = (image[:, 0:1, :, :] + 1.) / 2. # for visualization
            # if IM.shape[1] == 1:
            #     IM = IM.repeat(1, 3, 1, 1)
            # IM = IM.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            # mask = (image[:, 1:2, :, :] + 1.) / 2. # for visualization
            # mask = mask.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()


            # recon_img = (recon_out[:, 0:1, :, :] + 1.) / 2.
            # if recon_img.shape[1] == 1:
            #     recon_img = recon_img.repeat(1, 3, 1, 1)            
            # recon_img = recon_img.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            # recon_mask =(recon_out[:, 1:2, :, :]+ 1.) / 2.
            # recon_mask = recon_mask.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            recon_img = (recon_out + 1.) / 2.
            recon_img = recon_img.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

            recon_img = np.clip(recon_img, 0, 1)
            #recon_mask = np.clip(recon_mask, 0, 1)
            ...
            # cv2.imwrite(os.path.join(self.logdir, f"{step}_{idx}_Mask_output.png"), recon_mask* 255)
...
def train(
    ...
    tokenizer = AutoTokenizer.from_pretrained(pretrained_model_path, subfolder="tokenizer", use_fast=False) # BiomedCLIP Tokenizer
    text_encoder = AutoModel.from_pretrained(pretrained_model_path, subfolder="text_encoder") # CLIP Text Encoder#, trust_remote_code=True) # BiomedCLIP Text Encoder
    vae = AutoencoderKL.from_pretrained('/mnt/rds/VipinRDS/VipinRDS/users/cxs965/QK/MRGen/train_vae/PRGen_1024_512_251221-000649/checkpoint_35000', subfolder="")
    ...
    train_T2P_dataset = PRGenDataset(data_path=data_json, mode='train')
    val_T2P_dataset = PRGenDataset(data_path=data_json, mode='val')
    
    train_dataset = train_T2P_dataset
    val_dataset = val_T2P_dataset
    ...
    while step < train_steps:
        ...
        #     image = image * mask # set masked region to -1
        ...
        encoder_hidden_states = text_encoder(text_input_ids, attention_mask=attention_mask)[0]
...
if __name__ == "__main__":
    ...
    #CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --main_process_port 12345 train_unet_1024_512.py
